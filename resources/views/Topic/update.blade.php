...
<title>Edit Topic — {{ $room->name }}</title>
<style>
...
.page-badge{display:inline-flex;align-items:center;gap:6px;background:linear-gradient(135deg,#f59e0b,#fbbf24);color:#fff;padding:5px 16px;border-radius:20px;font-size:12px;font-weight:700;letter-spacing:.4px;}
...
.btn-primary{background:linear-gradient(135deg,#f59e0b,#fbbf24);color:#fff;width:100%;padding:13px;border-radius:12px;font-size:14px;box-shadow:0 4px 12px rgba(245,158,11,.25);}
.btn-primary:hover{background:linear-gradient(135deg,#d97706,#f59e0b);box-shadow:0 6px 16px rgba(245,158,11,.35);}
.btn-cancel{display:block;text-align:center;background:#f0f2f8;color:#5a6a8a;width:100%;padding:12px;border-radius:12px;font-size:14px;font-weight:700;text-decoration:none;transition:.2s;}
.btn-cancel:hover{background:#e4e8f0;color:#1a1a2e;}
.btn-start{background:linear-gradient(135deg,#27ae60,#2ecc71);color:#fff;width:100%;padding:13px;border-radius:12px;font-size:14px;box-shadow:0 4px 12px rgba(39,174,96,.25);}
...
.topic-item.active-edit{border-color:#f59e0b;background:#fef9ec;}
.topic-num{background:#e4e8f0;color:#5a6a8a;padding:3px 8px;border-radius:6px;font-size:12px;font-weight:800;flex-shrink:0;}
.topic-item.active-edit .topic-num{background:#f59e0b;color:#fff;}
...
<div class="page-header">
    <div>
        <div class="breadcrumb">
            ...
            <span>›</span>
            <a href="/show/{{ $room->id }}">{{ $room->name }}</a>
            <span>›</span>
            <span>Edit Topic</span>
        </div>
        <div class="page-title" style="margin-top:6px;">
            <span class="page-badge">✏️ Edit Topic</span>
            <span style="font-size:15px;color:#5a6a8a;font-weight:600;">{{ $topic->name }}</span>
        </div>
    </div>
</div>
    ...
    <div class="layout">
        ...
        <div class="card">
            <div class="card-header">
                ...
                <div>
                    <h2>Edit Topic</h2>
                    <p>Update the details for "{{ $topic->name }}"</p>
                </div>
            </div>
            <div class="card-body">
                <form method="POST" action="/rooms/{{ $room->id }}/topics/{{ $topic->id }}" id="editForm">
                    @csrf
                    ...
                    <div class="form-row col-3">
                        <div class="field">
                            ...
                            <input type="text" name="topic_name" id="topic_name" class="input"
                                   value="{{ old('topic_name', $topic->name) }}"
                        ...
                        <div class="field">
                            ...
                            <select name="duration" class="input" style="cursor:pointer;">
                                ...
                                <option value="{{ $val }}" {{ $topic->duration == $val ? 'selected' : '' }}>{{ $lbl }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div id="max_choices_row" style="display:{{ $isMulti ? 'block' : 'none' }};margin-bottom:14px;">
                        ...
                        <input type="number" name="max_choices" id="max_choices_input" class="input"
                               value="{{ $topic->max_choices }}" min="2" max="20" style="width:130px;">
                    </div>
                    ...
                    <div class="form-actions">
                        ...
                        <a href="/show/{{ $room->id }}" style="text-decoration:none;">
        ...
        <div style="display:flex;flex-direction:column;gap:16px;">
            ...
            <div class="card">
                <div class="card-body" style="padding:18px;">
                    <form method="GET" action="/rooms/{{ $room->id }}/start">
                    ...
                    <div style="margin-top:12px;padding:10px 14px;background:#f7f9fc;border-radius:10px;border:1px solid #e4e8f0;">
                        ...
                        <div style="font-size:20px;font-weight:800;color:#1a73e8;letter-spacing:4px;margin-top:2px;">{{ $room->code }}</div>
                    </div>
                </div>
            </div>
...
<script>
const EXISTING_CHOICES = @json($choixes);
...
window.onload = () => {
    if(EXISTING_CHOICES.length > 0) {
        EXISTING_CHOICES.forEach(c => createChoice(c.name));
    } else {
...
function submitEditForm() {
    ...
    if (valid) document.getElementById('editForm').submit();
}
        ...
        <div style="overflow-y:auto;flex:1;padding:16px 22px;">
            <div id="panel-days">
                <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px;">
                    ...
                    <label style="display:flex;flex-direction:column;align-items:center;gap:6px;padding:12px 6px;border:2px solid #e4e8f0;border-radius:10px;cursor:pointer;background:#fff;" onmouseover="this.style.borderColor='#1a73e8'" onmouseout="this.style.borderColor=this.querySelector('input').checked?'#1a73e8':'#e4e8f0'">
