...
.alert-success{ background:rgba(39,174,96,0.1); border:1px solid #27ae60; color:#27ae60; padding:12px; border-radius:8px; margin-bottom:15px; text-align:center; font-weight:bold; }
.alert-error{ background:rgba(231,76,60,0.1); border:1px solid #e74c3c; color:#e74c3c; padding:12px; border-radius:8px; margin-bottom:15px; font-weight:600; }
...
.btn-save{ background:#1a73e8; color:#fff; width:100%; margin-top:10px; }
.btn-save:hover{ background:#1558b0; }
.btn-cancel{ background:#f0f0f0; color:#666; width:100%; margin-top:8px; display:block; text-align:center; padding:12px; border-radius:10px; text-decoration:none; font-weight:bold; font-size:14px; }
.btn-cancel:hover{ background:#e0e0e0; }
.btn-add{ background:#e0e0e0; color:#1a1a2e; padding:8px 12px; }
.btn-add:hover{ background:#d0d0d0; }
.btn-remove{ background:#e74c3c; color:#fff; padding:8px 10px; flex-shrink:0; }
...
.page-label{ text-align:center; margin-bottom:16px; }
.page-label span{ display:inline-block; background:#e8f0fe; color:#1a73e8; padding:6px 24px; border-radius:20px; font-size:13px; font-weight:700; border:1.5px solid rgba(26,115,232,0.2); }
</style>
</head>
<body>
...
<div class="page-label">
    <span>Edit Topic — {{ $room->name }}</span>
</div>
...
<div class="wrapper">
    ...
    <div class="container">
        ...
        <!-- LEFT: Edit Form -->
        <div class="card left">
            <p style="font-size:16px;font-weight:800;color:#1a1a2e;margin-bottom:16px;"> Edit Topic</p>
            ...
            <form method="POST" action="/rooms/{{ $room->id }}/topics/{{ $topic->id }}" id="editForm">
                ...
                <input type="text" name="topic_name" class="input"
                       value="{{ old('topic_name', $topic->name) }}" required>
                ...
                <div id="max_choices_row" style="display:{{ $isMulti ? 'block' : 'none' }};margin-bottom:10px;">
                    ...
                    <input type="number" name="max_choices" id="max_choices_input" class="input"
                           value="{{ $topic->max_choices }}" min="2" max="20" style="width:120px;">
                </div>
                ...
                <select name="duration" class="input">
                    @foreach(['00:00:15'=>'15s','00:00:30'=>'30s','00:01:00'=>'1 min','00:02:00'=>'2 min','00:03:00'=>'3 min','00:05:00'=>'5 min'] as $val=>$lbl)
                    <option value="{{ $val }}" {{ $topic->duration == $val ? 'selected' : '' }}>{{ $lbl }}</option>
                    @endforeach
                </select>

                <button type="submit" class="btn-save" onclick="document.getElementById('editForm').submit()">💾 Save Changes</button>
            </form>

            <a href="/show/{{ $room->id }}" class="btn-cancel">✕ Cancel</a>
        </div>
        ...
        <div class="card right">
            ...
            <div class="question {{ $q->id == $topic->id ? 'active-edit' : '' }}">
                <div class="q-left">
